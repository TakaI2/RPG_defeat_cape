...
| # | アニメーション | 用途 | Mixamo検索 |
|---|---------------|------|-----------|
| 1 | Idle_Standing | 立ち待機 | "idle" |
| 2 | Walk | 歩行 | "walking" |
| 3 | **Kneel_Down** | 膝をつく遷移 | "kneeling down" |
| 4 | **Kneel_Idle** | 膝立ち待機 | "kneeling idle" |
| 5 | **Kneel_Up** | 立ち上がる | "standing up" |
...
|---|---------------|------|-----------|
| 6 | Run | 走行 | "running" |
| 7 | Crouch_Idle | しゃがみ | "crouching idle" |
| 8 | Sit_Down | 座る | "sitting down" |
| 9 | Sit_Idle | 座り待機 | "sitting idle" |
| 10 | Attack_Melee | 近接攻撃 | "sword slash" |
| 11 | Cast_Magic | 魔法詠唱 | "magic attack" |
| 12 | Hit_Reaction | 被ダメ | "hit reaction" |

### IKで代用する動作

| 動作 | IK種類 | アニメ不要の理由 |
|------|--------|-----------------|
| 前かがみ | Spine IK | 角度を連続調整可能 |
| つま先立ち | Foot IK | 高さを連続調整可能 |
| 手を伸ばす | Hand IK | 位置を自由に指定可能 |
| 顔を向ける | LookAt IK | ターゲット追従 |
| 顔を近づける | Head IK | オフセット調整可能 |
...
| Kiss | 特殊コマンド | 姿勢適応IK + Head IK | embarrassed |
...
| 6 | 複合行動テスト | 全機能統合 |
| 7 | NPCをくすぐる | 指制御, tickle, 表情反応, 体の揺れ |
| 8 | スイッチを押す | 指(Point), touch, IK |
| 9 | 寝ているキャラにキス | 姿勢検出, LeanOver, 覆いかぶさりIK ★NEW |
| 10 | 子供キャラにキス | 身長差検出, BendDown, かがみIK ★NEW |
...
### 変更履歴
- v1.3: アニメーション・IKハイブリッド方式、必要アニメ一覧を追加
- v1.2: Kissアクション詳細（姿勢・身長差対応）を追加
